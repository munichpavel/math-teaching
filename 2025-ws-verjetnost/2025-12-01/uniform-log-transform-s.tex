...
Next, if $ - \frac{1}{\lambda} \log(b) \leq y \leq - \frac{1}{\lambda} \log(b)$, we have $F_Y(y) = \frac{b - \exp(-\lambda y)}{b - a}$.
...
F_Y(y)=
    \begin{cases}
        ...
        \frac{b - \exp(-\lambda y)}{b - a} & \text{if }- \frac{1}{\lambda} \log(b) \leq y \leq - \frac{1}{\lambda} \log(b) \\
        1 & \text {if } y > - \frac{1}{\lambda} \log(a).
    \end{cases}
\end{equation}

We notice that $Y$ is a shifted exponential distribution.
